...
namespace cDynamicArray
{
	//直接照搬的固定数组然后改的malloc
	int main_CDA()
	{
		void init_array(int *arr, int name,int size, bool use_const_data);
		void remove_duplicates(int *arr, int *n);
		void print_array(const int *arr);
		void bubble_sort(int *arr, int n);
		void selection_sort(int *arr, int n);
		void print_arrOut(int *arrOut, int digits, short purpose);
		void intersection_slow(const int* arrA, int sizeA, const int* arrB, int sizeB);
		...
		{
			case 1:
				intersection_slow(arrA,sizeA, arrB,sizeB);break;
			case 2:
	...
	void intersection_slow(const int* arrA, int sizeA, const int* arrB, int sizeB)
	{
		int arrOut[sizeA>sizeB?sizeA:sizeB]={0};
		short digits = 0;
	...
	{
		int max_size = (sizeA < sizeB) ? sizeA : sizeB;
		int* arrOut = (int*)malloc(max_size * sizeof(int));
	...
	{
		int arrOut[sizeA>sizeB?sizeA:sizeB] = {0}; // 静态数组
		int i = 0, j = 0, digits = 0;
